...
DEFINE_SPADES_SETTING(keyBind1, "F1");
DEFINE_SPADES_SETTING(keyBind2, "F2");
DEFINE_SPADES_SETTING(keyBind3, "F3");
DEFINE_SPADES_SETTING(keyBind4, "F4");
DEFINE_SPADES_SETTING(keyBind5, "F5");
		...
		void Client::KeyEvent(const std::string &name, bool down) {
			...
			if (name == "Escape") {
				...
				if (world->GetLocalPlayer()) {
					...
					} else if (CheckKey(keyBind1, name) && down) {
						scriptedUI->sendBind1();
						scriptedUI->setIgnored(name);
						Handle<IAudioChunk> chunk =
						  audioDevice->RegisterSound("Sounds/Player/Flashlight.opus");
						audioDevice->PlayLocal(chunk, AudioParam());
					} else if (CheckKey(keyBind2, name) && down) {
						scriptedUI->sendBind2();
						scriptedUI->setIgnored(name);
						Handle<IAudioChunk> chunk =
						  audioDevice->RegisterSound("Sounds/Player/Flashlight.opus");
						audioDevice->PlayLocal(chunk, AudioParam());
					} else if (CheckKey(keyBind3, name) && down) {
						scriptedUI->sendBind3();
						scriptedUI->setIgnored(name);
						Handle<IAudioChunk> chunk =
						  audioDevice->RegisterSound("Sounds/Player/Flashlight.opus");
						audioDevice->PlayLocal(chunk, AudioParam());
					} else if (CheckKey(keyBind4, name) && down) {
						scriptedUI->sendBind4();
						scriptedUI->setIgnored(name);
						Handle<IAudioChunk> chunk =
						  audioDevice->RegisterSound("Sounds/Player/Flashlight.opus");
						audioDevice->PlayLocal(chunk, AudioParam());
					} else if (CheckKey(keyBind5, name) && down) {
						scriptedUI->sendBind5();
						scriptedUI->setIgnored(name);
						Handle<IAudioChunk> chunk =
						  audioDevice->RegisterSound("Sounds/Player/Flashlight.opus");
						audioDevice->PlayLocal(chunk, AudioParam());
					} else if (CheckKey(keyBind6, name) && down) {
						scriptedUI->sendBind6();
						scriptedUI->setIgnored(name);
						Handle<IAudioChunk> chunk =
						  audioDevice->RegisterSound("Sounds/Player/Flashlight.opus");
						audioDevice->PlayLocal(chunk, AudioParam());
